...
void create_registration_window(GtkWidget *login_window) {
    close_login_window(login_window);  // Закрытие окна авторизации
    ...
    // Кнопка "Назад" для возвращения в окно логина
    ...
    g_signal_connect(button_back, "clicked", G_CALLBACK(close_registration_window), window); // Переход на окно логина
    ...
    gtk_grid_attach(GTK_GRID(grid), button_back, 0, 3, 2, 1); // Добавление кнопки "Назад"
...
// Функция для создания окна логина
void create_login_window() {
    ...
    GtkWidget *label_register_prompt = gtk_label_new("Нет аккаунта? Зарегистрируйся");
    gtk_widget_set_margin_top(label_register_prompt, 10);

    // Кнопка для перехода на окно регистрации
    GtkWidget *button_register = gtk_button_new_with_label("Начать регистрацию");
    gtk_widget_set_margin_top(button_register, 5);

    // Привязка обработчиков к кнопкам
    GtkWidget *entries[2] = {entry_username, entry_password};
    g_signal_connect(button_login, "clicked", G_CALLBACK(on_login_clicked), entries);
    g_signal_connect(button_register, "clicked", G_CALLBACK(create_registration_window), window); // Передаем окно для скрытия
...
int main(int argc, char *argv[]) {
    ...
    create_login_window(); // Открытие окна авторизации
